...
def run(
    ...
    for i_episode in range(episodes):
        ...
        while not done: 
            ...
            step_result = env.step(action_numpy)
            # conduct action
            #next_state_numpy, reward, done, info = env.step(action_numpy)
            # İlk dört öğeyi al
            if len(step_result) == 5:
                next_state_numpy, reward, done, _, _ = step_result
            else:
                next_state_numpy, reward, done, _ = step_result
            score += reward
    ...
    print(f"Kodun çalışma süresi: {total_time:.2f} dakika")
    # save results
    output = {
